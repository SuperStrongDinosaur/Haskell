...
spec_unit = do
  ...
  describe "Parsers" $ do
    ...
    it "brackets \"(()\"" $
      runParser brackets "(()" `shouldBe` Nothing
    it "list \"2, 1,+10  , 3,5,-7, 2\"" $
      runParser lists "2, 1,+10  , 3,5,-7, 2" `shouldBe` Just ([ [1, 10], [5, -7, 2] ], "")
    it "list \"2, 1,+10, 0, 3,5,-7, 2\"" $
      runParser lists "2, 1,+10, 0  , 3,5,-7, 2" `shouldBe` Just ([ [1, 10], [], [5, -7, 2] ], "")
    it "list \"2, 1,+10, 0, 3,5,-7, 2\"" $
      runParser lists "   2   ,  1 , +10  , 0  , 3 ,     5 ,    -7     ,           2" `shouldBe` Just ([ [1, 10], [], [5, -7, 2] ], "")
